...
def render_markdown_with_data(
    ...
    try:
        ...
        # markdown_to_html() which runs the result through bleach (_sanitize_html)
        # before returning it — python-markdown itself does NOT sanitize raw HTML.
...
# Report markdown can legitimately produce these tags (tables, TOC anchors,
# fenced code, headings, ...) via the `markdown` extensions enabled below.
_ALLOWED_TAGS = [
...
def _sanitize_html(raw: str) -> str:
    """
    Sanitize markdown-generated HTML before it is rendered with `|safe`
    (see report_view.html) or returned directly as an HTMLResponse
    (report_builder.py's /builder/preview). Report content — including
    scan data pulled into the template context — is not fully trusted:
    a report author (any role with report_access) can type arbitrary
    "markdown" containing raw HTML, and python-markdown >=3.0 passes raw
    HTML straight through (safe_mode was removed). Using a real HTML
    parser (bleach, built on html5lib) instead of hand-rolled regexes —
    the previous approach missed well-known bypasses like `<svg/onload=..>`
    (no whitespace before the attribute) and encoded `javascript:` URIs.
    """
    return bleach.clean(
        raw,
        tags=_ALLOWED_TAGS,
...
def build_template_context(scan_data: Dict[str, Any], tenant: Any = None) -> Dict[str, Any]:
    ...
    if tenant:
        context["tenant"] = {
            ...
            # Full white-labeling (#48): even with a custom company name/logo
            # set above, several report sections still hardcoded "Report
            # generated by YADS" / "Assessment Tool: YADS Security Scanner"
            # regardless — defeating white-labeling for MSSPs reselling under
            # their own brand. This flag lets those specific lines be omitted.
            "hide_yads_branding": bool(getattr(tenant, 'hide_yads_branding', False)),
        }
    else:
        context["tenant"] = {
            "name": "YADS",
...
---
{% if not tenant.hide_yads_branding %}
...
{% if not tenant.hide_yads_branding %}**Assessment Tool:** YADS Security Scanner
{% endif %}**Report Generated:** {{ now|datetime }}
