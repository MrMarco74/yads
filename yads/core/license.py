...
class ActivationVerifier:
    """
    Verifies activation codes generated by scripts/sign_activation.py.
    Uses ACTIVATION_PUBLIC_KEY if configured, otherwise falls back to LICENSE_PUBLIC_KEY.
    Activation codes have a different payload format:
      {instance_uuid, customer_id, activated_at, exp, install_index}
    """
    ...
    def _load_public_key(self) -> Optional[ed25519.Ed25519PublicKey]:
        key_b64 = settings.ACTIVATION_PUBLIC_KEY or settings.LICENSE_PUBLIC_KEY
        if not key_b64:
            logger.warning("No ACTIVATION_PUBLIC_KEY or LICENSE_PUBLIC_KEY configured.")
            return None
        try:
            ...
            logger.error(f"Failed to load activation public key: {e}")
            return None
