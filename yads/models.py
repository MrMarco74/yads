...
class Tenant(SQLModel, table=True):
    ...
    hide_yads_branding: bool = Field(default=False)  # Full white-label (#48): suppress "Report generated by YADS" lines
...
class ReportSubscription(SQLModel, table=True):
    ...
    ScanSchedule (which controls *scanning* cadence) and from
    send_daily_digests (a fixed daily summary). This lets a tenant say
    "email this report every Monday to these recipients" independently.
    """
    id: Optional[int] = Field(default=None, primary_key=True)
    tenant_id: int = Field(foreign_key="tenant.id", index=True)
    name: str
    report_type: str = Field(default="executive_summary")  # executive_summary, findings_csv, compliance
    recipients: List[str] = Field(default=[], sa_column=Column(JSONB))
    frequency: str = Field(default="weekly")  # weekly, monthly
    day_of_week: int = Field(default=0)  # 0=Monday, used when frequency=weekly
    day_of_month: int = Field(default=1)  # used when frequency=monthly
    is_active: bool = Field(default=True)
...
class GeneratedReport(SQLModel, table=True):
    ...
    # report's external recipient, purely an internal approval workflow note.
...
class APIKey(SQLModel, table=True):
    ...
    """
    id: Optional[int] = Field(default=None, primary_key=True)
    # Nullable: Platform Admins (User.tenant_id=None) also need to be able to
    # own API keys. A NULL tenant_id key is intentionally scoped to nothing
    # by every `APIKey.tenant_id == Target.tenant_id`-style query (fails
    # closed, not open) until cross-tenant API key scoping is designed.
    tenant_id: Optional[int] = Field(default=None, foreign_key="tenant.id", index=True)
...
class IntegrationConfig(SQLModel, table=True):
    ...
    # Health-check badge (see yads/core/integration_health.py)
    last_check_at: Optional[datetime] = Field(default=None)
    last_check_status: Optional[str] = Field(default=None)   # "ok" | "failed" | "untested"
    last_check_message: Optional[str] = Field(default=None)
...
class SecurityFinding(SQLModel, table=True):
    ...
    snoozed_until: Optional[datetime] = Field(default=None)  # triage: temporarily hidden until this time

    # Recurrence tracking
    reopened_count: int = Field(default=0)

    # MITRE ATT&CK mapping (see yads/core/mitre_mapping.py) — real TAxxxx/Txxxx IDs,
    # distinct from SecurityAuditLog's mapping which only covers internal audit events.
    mitre_tactic_id: Optional[str] = Field(default=None, index=True)      # e.g. "TA0001"
    ...
    # is a legal judgment call, YADS only starts/tracks the clock.
    nis2_marked_at: Optional[datetime] = Field(default=None)
    nis2_deadline_24h: Optional[datetime] = Field(default=None)
    nis2_deadline_72h: Optional[datetime] = Field(default=None)
    nis2_marked_by: Optional[str] = Field(default=None)
...
class BaselineSnapshot(SQLModel, table=True):
    """
    Generic "last known state" store for point-in-time diffing (see
    yads/core/baseline_diff.py). One row per (tenant_id, target_id, snapshot_key);
    overwritten on every save — history is not kept here, only current-vs-previous.
    Used by: ASR delta, API-discovery baseline diff, whois/DNS ownership-change
    detection, AXFR recheck, wayback secret-diff, and similar "what changed since
    last time" features across otherwise-unrelated scanner modules.
    """
    __tablename__ = "baseline_snapshot"
    __table_args__ = (UniqueConstraint("tenant_id", "target_id", "snapshot_key", name="uq_baseline_snapshot"),)

    id: Optional[int] = Field(default=None, primary_key=True)
    tenant_id: Optional[int] = Field(default=None, foreign_key="tenant.id", index=True)
    target_id: Optional[int] = Field(default=None, foreign_key="target.id", index=True)
    snapshot_key: str = Field(index=True)   # e.g. "asr_open_ports", "api_discovery_endpoints"
    data: dict = Field(default={}, sa_column=Column(JSONB))
    updated_at: datetime = Field(default_factory=datetime.utcnow)
