...
- **`catchall_detector.py` läuft doppelt pro Scan** ⏸️ zurückgestellt (Final-Review-Fund, Parked-Domain-Detection-Branch, 2026-08-24): der neue immer-laufende Pre-Check in `worker_tasks.py` (`_run_parked_precheck` → `_check_parked_domain`) ruft `CatchallDetectorScanner.run_scan()` einmal für die Live-Gating-Entscheidung auf und dann erneut über `.process()` für die Persistierung — macht pro Scan bis zu ~8 HTTP-Requests (Ziel + Nonce-Vhost, je http/https) und potenziell zwei LLM-Klassifizierungen statt einer. Zusätzlich Konsistenz-Problem: bei knappen Fällen (Vhost-`SequenceMatcher` nahe 0.90-Schwelle, nicht-deterministisches LLM-Verdikt) können beide Läufe unterschiedlich ausfallen — Tag/Skip-Entscheidung nutzt Lauf 1, gespeichertes `ScanResult`/Score/UI-Badge nutzt Lauf 2, im Extremfall widersprüchlich. Sauberer Fix: ein `precomputed`/`raw_data`-Parameter auf `BaseScannerModule.process()`, sodass Verdikt und Persistierung dieselbe Beobachtung teilen — generische Verbesserung, nicht nur für `catchall_detector`, aber aus Zeitbudget-Gründen nicht Teil der ursprünglichen Feature-Umsetzung.

### Infra
- **`yads-api`-Image fehlen Pakete, die im `yads-worker`-Image vorhanden sind, obwohl beide nominell aus derselben `requirements.txt` bauen** ⏸️ zurückgestellt (Incident, Parked-Domain-Detection-Deploy, 2026-08-24): ein versehentlich eager (statt lazy) importiertes `CatchallDetectorScanner` in `worker_tasks.py` ließ `yads-api` beim Boot mit `ModuleNotFoundError: No module named 'bs4'` crash-loopen — `beautifulsoup4` steht in `requirements.txt` und ist im `yads-worker`-Image installiert, im `yads-api`-Image aber nicht. Sofort-Fix war, den Import wieder lazy zu machen (Konvention, die schon jeder andere Scanner-Modul-Import in dieser Datei befolgt) — das behebt den akuten Crash, aber nicht die zugrundeliegende Asymmetrie zwischen den beiden Docker-Build-Targets. Jeder zukünftige Scanner-Modul-Import, der versehentlich nicht lazy ist, kann denselben Crash reproduzieren. Sauberer Fix: `yads-infra`s Dockerfile/Compose-Setup prüfen, warum `api`- und `worker`-Target-Stage nicht dieselbe `requirements.txt`-Installation teilen, und angleichen.
